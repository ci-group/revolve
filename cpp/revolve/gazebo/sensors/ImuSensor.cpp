...
namespace revolve
{
  namespace gazebo
  {
    ...
    {
      this->castSensor_ =
              boost::dynamic_pointer_cast< gz::sensors::ImuSensor >(
                      this->sensor_);
      ...
      std::memset(lastValues_, 0, sizeof(lastValues_));

      // Add update connection that will produce new value
      this->updateConnection_ =
              this->castSensor_->ConnectUpdated(
    ...
    void ImuSensor::read(double *input)
    {
      ...
      std::memcpy(input, lastValues_, sizeof(lastValues_));
    }
  }  /* namespace gazebo */
}  /* namespace revolve */
