...
namespace revolve
{
  namespace gazebo
  {
    ...
    void BodyAnalyzer::OnModel(ConstModelPtr &msg)
    {
      ...
      {
        throw std::runtime_error("INTERNAL ERROR: Expecting model with name '"
                                 + expectedName + "', created model was '"
                                 + msg->name() + "'.");
      }
    ...
    void BodyAnalyzer::OnContacts(ConstContactsPtr &msg)
    {
      ...
      // https://bitbucket.org/osrf/gazebo/issue/1325/
      // getboundingbox-returns-the-models-last
      ...
      // Publish the message, serializing the response message in
      // the wrapper data
      response.SerializeToString(wrapper.mutable_serialized_data());
