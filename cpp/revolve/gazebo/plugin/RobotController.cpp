  ...
  namespace gazebo
  {
                               ...
                               sdf::ElementPtr _sdf)
    {
      ...
      batterySetSub_ = node_->Subscribe("~/battery_level/request",
                                        ...
                                        this);
      batterySetPub_ =
              node_->Advertise< gz::msgs::Response >(
                      "~/battery_level/response");

      if (not _sdf->HasElement("rv:robot_config"))
      {
        std::cerr << "No `rv:robot_config` element found, "
                "controller not initialized." << std::endl;
        return;
      }
      ...
      {
        double updateRate =
                settings->GetElement("rv:update_rate")->Get< double >();
...
/////////////////////////////////////////////////
    ...
    {
      if (request->data() != this->model->GetName()
          && request->data() != this->model->GetScopedName())
      {
        return;
      }
...
/////////////////////////////////////////////////
    ...
    void RobotController::LoadBrain(sdf::ElementPtr sdf)
    {
      ...
      brain_.reset(new NeuralNetwork(this->model->GetName(),
                                     brain,
                                     motors_,
                                     sensors_));
    }
