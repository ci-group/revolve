...
namespace revolve
{
  namespace gazebo
  {
    WorldController::WorldController() :
            robotStatesPubFreq_(0), lastRobotStatesUpdateTime_(0)
    {
    }
                               ...
                               sdf::ElementPtr /*_sdf*/)
    {
      ...
      responseSub_ = node_->Subscribe("~/response",
      ...
      modelSub_ =
              node_->Subscribe("~/model/info", &WorldController::OnModel, this);
      ...
      robotStatesPub_ = node_->Advertise< revolve::msgs::RobotStates >(
...
// Process insert and delete requests
    void WorldController::HandleRequest(ConstRequestPtr &request)
    {
      if (request->request() == "delete_robot")
      {
        ...
        if (model)
        {
          ...
          // something to do with race conditions where the model is used by the
          // world while it is being updated. Fixing this completely appears to
          // be a rather involved process, instead, we'll use an `entity_delete`
          // request, which will make sure deleting the model happens on the
          // world thread.
      ...
      {
        std::cout
                << "Processing insert model request ID `"
                << request->id()
                << "`."
                << std::endl;
        sdf::SDF robotSDF;
      ...
      {
        robotStatesPubFreq_ =
                boost::lexical_cast< unsigned int >(request->data());
        std::cout
                << "Setting robot state update frequency to "
                << robotStatesPubFreq_
                << "."
                << std::endl;
    ...
    void WorldController::OnModel(ConstModelPtr &msg)
    {
      ...
      std::cout << "Model `" << name
                << "` inserted, world now contains " << world_->GetModelCount()
                << " models." << std::endl;
    }
