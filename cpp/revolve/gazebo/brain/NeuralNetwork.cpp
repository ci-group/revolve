...
namespace revolve
{
  namespace gazebo
  {
              ...
              , nNonInputs_(0)
    {
      ...
      // prevent SDF from complaining
      // if no neurons are present.
      auto neuron = node->HasElement("rv:neuron") ?
                    node->GetElement("rv:neuron") :
                    sdf::ElementPtr();
      while (neuron)
      {
      ...
      while (connection)
      {
        if (not connection->HasAttribute("src") || not connection->HasAttribute(
                "dst")
            || not connection->HasAttribute("weight"))
        {
          std::cerr << "Missing required connection attributes "
                  "(`src`, `dst` or `weight`)." << std::endl;
          throw std::runtime_error("Robot brain error");
        }
    ...
    void NeuralNetwork::step(double time)
    {
      ...
      for (i = 0; i < nNonInputs_; ++i)
      {
        ...
        {
          curNeuronActivation +=
                  hiddenWeights_[maxNonInputs * j + i]
                  * curState[nOutputs_ + j];
        }
...
/////////////////////////////////////////////////
                      ...
                      sdf::ElementPtr neuron)
    {
      ...
      if ("Sigmoid" == type || "Simple" == type)
      {
        ...
        if (not neuron->HasElement("rv:bias") || not neuron->HasElement(
                "rv:gain"))
        {
          std::cerr
                  << "A `"
