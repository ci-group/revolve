...
class Plasticoding(Genotype):
    ...
    def __init__(self, conf, robot_id=0):
        """
    ...
    def late_development(self):
        ...
        for symbol in self.intermediate_phenotype:
            ...
            if symbol[self.index_symbol] == Alphabet.CORE_COMPONENT:
                ...
                module.info = {'orientation': Orientation.FORWARD,
    ...
    def move_in_body(self, symbol):
        ...
        if symbol[self.index_symbol] == Alphabet.MOVE_BACK \
            ...
            self.mounting_reference_stack.pop()
                ...
                and self.mounting_reference.children[Orientation.FORWARD.value] is not None:
            if type(self.mounting_reference.children[Orientation.FORWARD.value]) is not TouchSensorModule:
                self.mounting_reference_stack.append(self.mounting_reference)
                self.mounting_reference = \
                    self.mounting_reference.children[Orientation.FORWARD.value]

        elif symbol[self.index_symbol] == Alphabet.MOVE_LEFT \
                and type(self.mounting_reference) is not ActiveHingeModule:
            if self.mounting_reference.children[Orientation.LEFT.value] is not None:
                if type(self.mounting_reference.children[Orientation.LEFT.value]) is not TouchSensorModule:
                    self.mounting_reference_stack.append(self.mounting_reference)
                    self.mounting_reference = \
                        self.mounting_reference.children[Orientation.LEFT.value]

        elif symbol[self.index_symbol] == Alphabet.MOVE_RIGHT \
                and type(self.mounting_reference) is not ActiveHingeModule:
            if self.mounting_reference.children[Orientation.RIGHT.value] is not None:
                if type(self.mounting_reference.children[Orientation.RIGHT.value]) is not TouchSensorModule:
                    self.mounting_reference_stack.append(self.mounting_reference)
                    self.mounting_reference = \
                        self.mounting_reference.children[Orientation.RIGHT.value]
