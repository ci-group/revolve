        ...
        for i in range(0, n_iterations):
            ...
            for aux_index in range(0, len(developed_sentence)):
                ...
                if symbol in Alphabet.modules(self._conf.allow_vertical_brick):
                    ...
                    developed_sentence.insert(position, (Alphabet.PUSH_MOV_STACK, []))

                    ii = 0
                    for ii in range(0, len(self._grammar[symbol])):
                        developed_sentence.insert(position + ii + 1, self._grammar[symbol][ii])

                    developed_sentence.insert(position + ii + 2, (Alphabet.POP_MOV_STACK, []))

                    position = position + ii + 3
                else:
                    position = position + 1
...
class PlasticodingDecoder:
        ...
        for word in self._command_sentence[1:]:
            ...
            if symbol in Alphabet.morphology_mounting_commands():
                ...
                if self.morph_mounting_container is not None \
                    ...
                    or symbol in [Alphabet.POP_MOV_STACK, Alphabet.PUSH_MOV_STACK]:
    ...
    def get_slot(morph_mounting_container):
        ...
        elif morph_mounting_container == Alphabet.ADD_BACK:
            slot = Orientation.SOUTH
        return slot
    ...
    def get_color(new_module_type):
        if Alphabet.is_block(new_module_type):
            rgb = [0.0, 0.0, 1.0]
    ...
    def move_in_body(self, word):
        ...
        elif symbol is Alphabet.MOVE_NONE:
            self.stack.current.module = self.stack.current.module
        else:
    ...
    def new_module(self, slot, new_module_type: Alphabet, word, allow_joint_joint_attachment: bool):
        ...
        if Alphabet.is_block(new_module_type) or new_module_type == Alphabet.BLOCK_VERTICAL:
            module = BrickModule()
        elif new_module_type == Alphabet.JOINT_VERTICAL \
        ...
        if new_module_type != Alphabet.SENSOR:
            try:
                ...
                self.morph_mounting_container = None
                self.stack.save_history()
                self.stack.current.module = module #TODO undeclared
