...
import pyrevolve.URDF.math
from pyrevolve.custom_logging.logger import logger


class Pose(xml.etree.ElementTree.Element):
    def __init__(self, position: URDF.math.Vector3 = None, rotation: URDF.math.Quaternion = None):
        self.position = URDF.math.Vector3() if position is None else position
        self.rotation = URDF.math.Quaternion() if rotation is None else rotation

        roll, pitch, yaw = self.rotation.get_rpy()
        x, y, z = self.position.x, self.position.y, self.position.z
        super().__init__('origin', {'xyz': '{:e} {:e} {:e}'.format(x, y, z),
                                    'rpy': '{:e} {:e} {:e}'.format(roll, pitch, yaw)})
    ...
    def text(self):
        if len(self) < 1:
            roll, pitch, yaw = self.rotation.get_rpy()
            x, y, z = self.position.x, self.position.y, self.position.z
            # xml.etree.ElementTree.SubElement(self, 'origin', {'rpy': '{:e} {:e} {:e}'.format(roll, pitch, yaw),
            #                                                   'xyz': '{:e} {:e} {:e}'.format(x, y, z)})
            # self.__delitem__('origin')
            self.set('rpy', '{:e} {:e} {:e}'.format(roll, pitch, yaw))
            self.set('xyz', '{:e} {:e} {:e}'.format(x, y, z))
        return
...
class Posable(xml.etree.ElementTree.Element):
    def __init__(self, tag, attrib={}, position=None, rotation=None, **extra):
        ...
        # xml.etree.ElementTree.SubElement(self, 'rv:controller', {'type': 'None'})
    ...
    def get_rotation(self):
        ...
        :rtype: URDF.math.Quaternion
        """
        return self._pose.rotation.copy()

    def set_position(self, position: URDF.math.Vector3):
        self._pose.position = position.copy()

    def set_rotation(self, rotation: URDF.math.Quaternion):
        self._pose.rotation = rotation.copy()

    def translate(self, translation: URDF.math.Vector3):
        self._pose.position = self._pose.position + translation

    def rotate(self, rotation: URDF.math.Quaternion):
        self._pose.rotation = rotation * self._pose.rotation

    def rotate_around(self, axis: URDF.math.Vector3, angle: float, relative_to_child: bool = True):
        """
        ...
        quat = URDF.math.Quaternion.from_angle_axis(angle, axis)
        self.rotate(quat)
    ...
    def to_parent_direction(self, vec):
        ...
        :type vec: URDF.math.Vector3|URDF.math.Quaternion
        :return:
        :rtype: URDF.math.Vector3|URDF.math.Quaternion
        """
    ...
    def to_parent_frame(self, point: URDF.math.Vector3):
        """
    ...
    def to_local_frame(self, point: URDF.math.Vector3):
        """
    ...
    def to_sibling_frame(self, point: URDF.math.Vector3, sibling):
        """
    ...
    def align(self,
        ...
        if not relative_to_child:
            ...
            # my.pos = of.to_local_frame(at.pos)
            # my.normal = of.to_local_direction(at.normal)
            # my.tangent = of.to_local_direction(at.tangent)
        ...
        r1 = URDF.math.RotationMatrix()
        r2 = URDF.math.RotationMatrix()
