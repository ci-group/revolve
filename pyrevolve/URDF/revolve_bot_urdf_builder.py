...
def revolve_bot_to_urdf(robot, robot_pose, nice_format, self_collide=True):
    ...
    core_link = URDF.Link('Core', self_collide=self_collide)
    core_visual, core_collision, imu_core_sensor = robot._body.to_urdf('', core_link)
    ...
    for core_slot, child_module in robot._body.iter_children():
        ...
        children_joints, \
        children_sensors, \
        children_collisions = _module_to_urdf(child_module,
                                              core_link,
                                              core_slot,
                                              core_collision,
                                              slot_chain,
    ...
    # ADD BRAIN
    plugin_elem = _urdf_brain_plugin_conf(robot._brain, sensors, actuators, robot_genome=None)
    ...
    res = xml.etree.ElementTree.tostring(model, encoding='utf8', method='xml')
...
def _urdf_attach_module(module_slot, module_orientation: float,
                        visual, collision,
    ...
    :return:
    """
    from pyrevolve.SDF.math import Quaternion
    if module_orientation is not None:
        ...
        rot = Quaternion.from_angle_axis(orientation, module_slot.normal)
...
def _module_to_urdf(module, parent_link, parent_slot: BoxSlot, parent_collision, slot_chain, self_collide):
    ...
    :param parent_link: URDF `Link` of the parent
    ...
    if type(module) is ActiveHingeModule:
        child_link = URDF.Link('{}_Leg'.format(slot_chain), self_collide=self_collide)

        visual_frame, collisions_frame, \
        visual_servo, collisions_servo, \
        joint = module.to_urdf('{}'.format(slot_chain), parent_link, child_link)

        # parent_slot = parent_module.boxslot(parent_slot)
        module_slot = module.boxslot_frame(Orientation.BACK)
        _urdf_attach_module(module_slot, module.orientation,
                            visual_frame, collisions_frame[0],
                            parent_slot, parent_collision)

        rotation_axis = visual_frame.get_rotation()
        joint_origin_rotation = visual_servo.get_rotation()
        joint_origin_position = visual_servo.get_position()
        ...
        joint.set_rotation(rotation_axis)
        joint.set_position(visual_servo.get_position())
        joint.translate(joint.to_parent_direction(old_position))

        visual_servo.set_rotation(joint_origin_rotation)
        visual_servo.set_position(joint_origin_position - old_position)

        collisions_servo[0].set_rotation(joint_origin_rotation)
    ...
    # OTHERS
        ...
        visual, collision, sensor = module.to_urdf(slot_chain, my_link)
...
def _urdf_brain_plugin_conf(
    ...
    :return: The urdf plugin element
    :rtype: xml.etree.ElementTree.Element
    """
    gazebo = xml.etree.ElementTree.Element('gazebo')
    plugin = xml.etree.ElementTree.SubElement(gazebo,
                                              'plugin',
                                              attrib={
    ...
    # update rate
    URDF.sub_element_text(config, 'rv:update_rate', update_rate)

    # battery
    # if battery_level is not None:
    #     battery = xml.etree.ElementTree.SubElement(config, 'rv:battery')
    #     URDF.sub_element_text(battery, 'rv:level', battery_level)

    # brain
    robot_brain_urdf = xml.etree.ElementTree.SubElement(config, 'rv:brain')

    robot_learner = robot_brain.learner_sdf()
    if robot_learner is None:
        xml.etree.ElementTree.SubElement(robot_brain_urdf, 'rv:learner', {'type': 'None'})
    else:
    ...
    if robot_controller is None:
        ...
        robot_brain_urdf.append(robot_controller)
    ...
    # sensors
    sensors_elem = xml.etree.ElementTree.SubElement(robot_brain_urdf, 'rv:sensors')
    for sensor in sensors:
        sensors_elem.append(sensor.to_robot_config_sdf())

    # actuators
    actuators_elem = xml.etree.ElementTree.SubElement(robot_brain_urdf, 'rv:actuators')
    for actuator in actuators:
        actuators_elem.append(actuator.to_robot_config_sdf())

    # robot genome
    if robot_genome is not None:
        URDF.sub_element_text(config, 'rv:genome', str(robot_genome))
