/*
* Copyright (C) 2019 Vrije Universiteit Amsterdam
*
...
#include "LearningWorldController.h"
...
/////////////////////////////////////////////////
LearningWorldController::LearningWorldController()
        : delete_robot_queue()
...
/////////////////////////////////////////////////
void LearningWorldController::Load(
        ...
        sdf::ElementPtr /*_sdf*/)
{
    ...
    std::cout << "LEARNING World plugin loaded." << std::endl;
    ...
    this->requestSub_ = this->node_->Subscribe(
            "~/request",
            &LearningWorldController::HandleRequest,
            this);
    ...
    this->responseSub_ = this->node_->Subscribe(
            "~/response",
            &LearningWorldController::HandleResponse,
            this);
    ...
    this->modelSub_ = this->node_->Subscribe(
            "~/model/info",
            &LearningWorldController::OnModel,
            this);
...
void LearningWorldController::Reset()
{
    this->lastRobotStatesUpdateTime_ = 0; //this->world_->SimTime().Double();
}

//////////// UPDATE PER EVALUATION //////////////
//void LearningWorldController::OnBeginUpdate(const ::gazebo::common::UpdateInfo &_info) {
//
//}
...
void LearningWorldController::OnBeginUpdate(const ::gazebo::common::UpdateInfo &_info) {
    ...
    if ((time - this->lastRobotStatesUpdateTime_) >= secs) {
        ...
        {
            boost::recursive_mutex::scoped_l<ock lock_physics(*this->world_->Physics()->GetPhysicsUpdateMutex());
    ...
    // for every model in models_to_remove, remove them
...
//            this->requestPub_->Publish(deleteReq);
    ...
    // erases all models_to_remove elements
    this->models_to_remove.clear();
//        this->world_insert_remove_mutex.unlock();
//    }
}

void LearningWorldController::OnEndUpdate()
{
...
void LearningWorldController::HandleRequest(ConstRequestPtr &request)
{
...
/////////////////////////////////////////////////
void LearningWorldController::OnModel(ConstModelPtr &msg)
{
    auto name = msg->name();
    std::cout << "LearningWorldController::OnModel(" << name << ')' << std::endl;
...
/////////////////////////////////////////////////
void LearningWorldController::HandleResponse(ConstResponsePtr &response)
{
//    std::cout << "LearningWorldController::HandleResponse(" << response->request() << ')' << std::endl;
