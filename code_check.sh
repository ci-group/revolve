...
then
  ...
  CHECK_DIRS="cpp"
# "./plugins ./gazebo ./tools ./examples ./test/integration"\
...
# " ./test/cmake ./test/pkgconfig"
  if [ $CPPCHECK_LT_157 -eq 1 ]; then
    ...
    CPPCHECK_FILES=`find $CHECK_DIRS -name "*.cpp"`
  else
    CPPCHECK_FILES=`find $CHECK_DIRS -name "*.cpp" -o -name "*.hh"`
  fi
  CPPLINT_FILES=`\
    find $CHECK_DIRS -name "*.cpp" -o -name "*.hh" -o -name "*.c" -o -name "*.h" | grep -v test_fixture/gtest`
fi
