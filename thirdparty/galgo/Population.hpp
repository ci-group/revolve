...
namespace galgo
{
  ...
  template <typename T> class Population
  {
    ...
    void evolution(size_t current_iteration);
    ...
    void recombination(size_t current_iteration);

    // complete new population randomly
    void completion(size_t current_iteration);
...
/////**** the interface to bayesian optimization ****///
  ...
  template <typename T> void Population< T >::evolution(size_t current_iteration)
  {
    ...
    this->recombination(current_iteration);

    // completing new population // only crossover
    // matpop[] -> newpop[nbrcrov...popsize]
    this->completion(current_iteration);
...
//    std::cout << "curpop.begin(): " << curpop[1]->get_value(0) << std::endl;
  ...
  // create new population from recombination of the old one
  template <typename T> void Population< T >::recombination(size_t current_iteration)
  {
...
#pragma omp parallel for num_threads(MAX_THREADS)
    ...
    for (int i = ptr->elitpop; i < nbrcrov; i = i + 2)
    {
      ...
      ptr->Mutation(newpop[i], current_iteration);
      ptr->Mutation(newpop[i + 1], current_iteration);
...
//      newpop[i + 1]->evaluate();
  ...
  template <typename T> void Population< T >::completion(size_t current_iteration)
  {
#ifdef _OPENMP
#pragma omp parallel for num_threads(MAX_THREADS)
#endif
