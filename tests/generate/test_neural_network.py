...
class TestNeuralNetGenerator(unittest.TestCase):
    ...
    def test_valid(self):
        ...
        self.assertTrue(brain.IsInitialized(),
                        "Incomplete brain (seed %d)." % seed)
        ...
        for neuron_id in inputs:
            ...
            self.assertTrue(len(neurons) == 1,
                            "Neuron %s not present." % neuron_id)
            self.assertEquals("input", neurons[0].layer,
                              "Neuron %s should have 'input' layer (seed %d)."
                              % (neuron_id, seed))
        ...
        for neuron_id in outputs:
            ...
            self.assertTrue(len(neurons) == 1,
                            "Neuron %s not present." % neuron_id)
            self.assertEquals("output", neurons[0].layer,
                              "Neuron %s should have 'output' layer (seed %d)."
                              % (neuron_id, seed))
            self.assertEquals("Oscillator", neurons[0].type,
                              "Output neuron %s should all be "
