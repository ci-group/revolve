...
async def run():
    ...
    simulator_queue = CeleryPopulationQueue(args, use_isaacgym=True, local_computing=True, use_db=False)
    ...
    #load_robot_pose(experiment_management.generation_folder(gen_num-1), population.individuals)
...
def load_robot_pose(generation_folder, individuals: List[Individual]) -> None:

    initial_pos_map = {}
    with open(os.path.join(generation_folder, 'extra.tsv')) as extra_file:
        for i, line in enumerate(extra_file):
            print(line, end='')
            if i == 0:
                continue
            line = line.split('\t')
            _id = int(line[0])
            initial_pos = line[1][1:-1].split(',')
            final_pos = line[2][1:-1].split(',')
            initial_pos_map[_id] = initial_pos

    for individual in individuals:
        _id = individual.id
        initial_pos: Tuple[str, str] = initial_pos_map[_id]
        individual.pose.x = float(initial_pos[0])
        individual.pose.y = float(initial_pos[1])
        print(f"load_robot_pose:{individual.id} {individual.pose}")
