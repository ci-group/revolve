...
analysis = 'analysis_master'
output_directory = paste(base_directory,'/',analysis ,sep='')

experiments_type = c(
  'evo_only',
  'evo_only_100offspring'
)

initials = c('p', 'l')

experiments_labels = c(  'evo_only',
                         'evo_only_100offspring')
...
for (exp in 1:length(experiments_type))
{
  for(run in runs)
  {
    ...
    measures_snapshots$fitness =   measures_snapshots$fitness*100
...
file_name=paste(base_directory,"summary_master.csv",sep = '/')
