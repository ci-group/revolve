...
analysis = 'noveltysmall2'
output_directory = paste(base_directory,'/',analysis ,sep='')

experiments_type = c('fnl_baseline', 'fnl_plastic')
runs = list(c(1:3), c(1:3))
environments = list( c( 'plane', 'tilted5'), c( 'plane', 'tilted5') ) # update with desired environment
initials =   c( 'bp', 'bt', 'pp', 'pt')
experiments_labels2 = c('fnl_baseline-plane', 'fnl_baseline-tilted','fnl_plastic-plane','fnl_plastic-tilted')

comps_combinations = list(
                        c('fnl_baseline-plane', 'fnl_baseline-tilted'),
                        c('fnl_plastic-plane', 'fnl_plastic-tilted'),
                        c('fnl_baseline-plane','fnl_plastic-plane'),
                        c('fnl_baseline-tilted','fnl_plastic-tilted')
                        )
...
# one color per item in experiments_labels2, but are not respected if show_legends as on. also the order of the colors is strange
experiments_type_colors = c( '#00e700' , '#ff9933', '#009900', '#cc0000')  
      ...
      if (all_na == FALSE)
      {
        ...
        if(type_summary == 'means')
        {
          if(show_legends == TRUE){
            graph = graph + geom_line(aes_string(y=paste(methods[m],'_',measures_names[i],'_avg',sep=''), colour=shQuote(experiments_labels2[m]) ), size=2)
          }else{
          ...
          if(show_legends == TRUE){
            graph = graph + geom_line(aes_string(y=paste(methods[m],'_',measures_names[i],'_median',sep='') , colour=shQuote(experiments_labels2[m])   ),size=2 )
          }else{
...
for (i in 1:length(measures_names))
{
  ...
  for (exp in 1:length(methods))
  {
    ...
    temp$type = experiments_labels2[exp]
  ...
  g1 <-  ggplot(data=all_final_values, aes(x= type , y=values, color=type )) +
    ...
    labs( x="Method-Season", y=measures_labels[i], title='')
  ...
  g1 = g1 + geom_signif( test="wilcox.test", size=2, textsize=22,
                         comparisons = comps_combinations,
