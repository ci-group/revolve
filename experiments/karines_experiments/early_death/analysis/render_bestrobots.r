...
environments = list(
  c( 'plane', 'tilted5') , c( 'plane', 'tilted5'), c( 'plane', 'tilted5')
)

colors = list( c( '#ff99ff', '#c0c0c0'), c('#ff99ff', '#c0c0c0') , c('#ff99ff', '#c0c0c0')  )

base_directory <- paste('/storage/karine/early_death/', sep='')

analysis = 'imagesbodies'
    ...
    for (g in 1:length(gens))
    {
      ...
      for(b in 1:nrow(bests))
      {
        ...
        for (env in 1:length(environments[[m]]))
        {
          ...
          body = image_border(image_background(body, "transparent" ),"transparent" , "5x5")
